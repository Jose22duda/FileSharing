...
class UDPClient(QThread):
    ...
    def __init__(self, tracker_host, tracker_port):
        """ Initialize the UDP client with tracker details """
        super().__init__()
        try:
    ...
    def run(self):
        ...
        try:
            ...
            if response.startswith("OK"):
                ...
                self.log_message.emit("Connected to Tracker")
                self.log_message.emit("Registered as a Seeder")
                ...
                self.log_message.emit("[+] You did not prived Resources\n[+] Add files to share in you Resource folder")

            elif response.startswith("Message Format Incorrect"):

                self.connected.emit(False)
                self.log_message.emit("[+] message format is incorrect")
                print("[+] message format is incorrect")

        except Exception as e:
            print(f"[ERROR] Failed to register with the tracker: {e}")
            self.log_message.emit(f"[ERROR] Failed to register with the tracker: {e}")
        ...
        try:
            while self.running:
                ...
                if response.startswith("SET SERVER"):
                    ...
                    self.log_message.emit("You have been unregistered as a Seeder by the Tracker")
                    self.running = False
    ...
    def setUPTcpServer(self, message):
        ...
        try:
            ...
            self.log_message.emit("f[*] Listening on port {response}...")

            conn, addr = TCP_server_socket.accept()   # Accept incoming connection
            print(f"[+] Connection established with {addr}")

            self.log_message.emit(f"[+] Connection established with {addr}")
            ...
            self.log_message.emit(f"[*] Chunk ID: {response[2]} requested.")
            self.log_message.emit(f"[*] Size of chunk: {response[1]} bytes")
            self.log_message.emit(f"[*] Client requested chunk of the following file {response[0]}")
            ...
            if not os.path.exists(filename):
                ...
                with open(filename, "rb") as f:
                    ...
                    if (chunk_size > 1024 * 1024 * 100):  # if file > 100MB
                        ...
                        self.log_message.emit(f"[*] SPLITING CHUNK ")
                        self.log_message.emit(f"[*] Leecher response to SPLIT CHUNK?: {message}")
                        ...
                        self.log_message.emit(f"[*] Preparing to send small chunks")
                        ...
                        self.log_message.emit(f"SENDING WHOLE CHUNK")
                        conn.send(message.encode())
            ...
            self.log_message.emit(f"[+] Chunk ID: {chunk_id} chunk was sent")
